...
public class SplashScr : mScreen
{
	...
	public override void update()
	{
		if (splashScrStat == 30 && !isCheckConnect)
		{
			...
			{
				GameCanvas.isPlaySound = ((Rms.loadRMSInt("isPlaySound") == 1) ? true : false);
			}
	...
	public static void loadIP()
	{
		...
		if ((text == null || text.Equals(string.Empty)) && (text2 == null || text2.Equals(string.Empty)))
		{
			...
			if (ServerListScreen.serverPriority == -1)
			{
				ServerListScreen.ipSelect = num2 + Res.random(0, ServerListScreen.lengthServer[mResources.language]);
			}
			else
			{
				ServerListScreen.ipSelect = ServerListScreen.serverPriority;
